...
public:
	...
	void mill() override {
		if (this->ledgeSides[1] && this->ledgeSides[3]) {
			...
			else std::cout << "Фрезерование змейкой с правого нижнего угла" << std::endl;
		}
		else {
			if (this->ledgeSides[2]) std::cout << "Фрезерование змейкой с левого верхнего угла" << std::endl;
			else std::cout << "Фрезерование змейкой с левого нижнего угла" << std::endl;
		}
	}
};
